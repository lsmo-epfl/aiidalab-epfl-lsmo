...
class MofCheckerWidget(ipw.VBox):
    ...
    def __init__(self, title=""):
        ...
        self.missing_h_button = ipw.Button(
            description="Add", layout={"width": "initial"}
        )
        ...
        with self._output:
            ...
            for check in self.checks:
                if not check.is_ok:
                    ...
                    button = ipw.Button(
                        description="Select", layout={"width": "initial"}
                    )
                    button.on_click(
            ...
            if (
                self.mfchk.undercoordinated_c_candidate_positions
                ...
                display(
                    ipw.HBox(
                        [ipw.HTML("Missing hydrogens found"), self.missing_h_button]
                    )
                )
...
class CheckMofStructure(ipw.VBox):
    ...
    def __init__(self, **kwargs):
        self.text = ipw.HTML()
        self.check_button = ipw.Button(
            description="Final Structure Check", button_style="warning"
        )
